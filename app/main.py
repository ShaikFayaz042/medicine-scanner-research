...
def list_documents():
    """
    Return all documents sorted newest-first by PARSED release date.
    Ties broken by document_id ascending (matches CDSCO's own order).
    """
    db = SessionLocal()
    try:
        ...
        # Two stable passes:
        #   1) sort by document_id ascending
        #   2) sort by parsed date descending
        # Python's sort is stable, so step 2 preserves step-1 order for ties.
...
def serve_pdf(document_id: int):
    ...
    # No 'filename=' → browser displays inline instead of downloading.
